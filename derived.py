...
def to_dnf_clause(cube:  List[int]) -> 'AndOp':
    return And(*map(to_dnf_var, cube))


def _get_derived_by(easy: List[int]) -> Clauses:
...
def get_derived_by(easy: List[int]) -> Tuple[List[int], List[List[int]]]:
    one_lit, constraints = [], []
